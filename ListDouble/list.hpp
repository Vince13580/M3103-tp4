...
void CList<T>::Show () const
{
    ...
    for (std::shared_ptr<CNode<T>> Ptr (m_fictionalHead -> GetDroite()); Ptr != m_fictionalTail; Ptr = Ptr->GetDroite ())
...
{
    std::shared_ptr<CNode<T>> Ptr (m_fictionalHead->GetDroite ());
    for (; Ptr != m_fictionalTail && Ptr -> GetData () != val ; Ptr = Ptr->GetDroite ());
...
{
    std::shared_ptr<CNode<T>> tmp (new CNode<T> (val, ptr, ptr-> GetDroite()));
    ptr -> SetNextNode (tmp);
    tmp->GetDroite () ->SetPrevNode (tmp);
}

template <typename T>
void CList<T>::AddBefore (const std::shared_ptr<CNode<T>> & ptr, const T & val)
{
    AddAfter(ptr->GetGauche(), val);
}
...
void CList<T>::Remove (std::shared_ptr<CNode<T>> & pDelete)
{
    ...
    for ( ; pFind -> GetDroite () != pDelete ; pFind = pFind -> GetDroite ());

    pFind -> SetNextNode (pDelete -> GetDroite ());
    pDelete -> GetDroite () -> SetPrevNode (pFind);
